...
function HomePage() {
  ...
  const thisMonth = bills.find((b) => b.month === "2025-06");
  const lastMonth = bills.find((b) => b.month === "2025-05");
  const delta = thisMonth && lastMonth ? thisMonth.total_bill_eur - lastMonth.total_bill_eur : 0;
  const savedVsLast = -delta; // positive = saved
  const monthLabel = new Date(today.date).toLocaleDateString("en-GB", { month: "long" });

  // Pick one insight to show — prefer high severity anomaly, else first
  const featured =
    insights.find((i) => i.severity === "high") ?? insights[0];
    ...
    <AppShell>
      <div className="space-y-8">
        ...
        <section className="grid sm:grid-cols-2 gap-4">
          <Link to="/chat" className="card-soft p-5 flex items-center justify-between hover:shadow-lg transition group">
          ...
          <Link to="/chat" search={{ q: "Show me my contract details" }} className="card-soft p-5 flex items-center justify-between hover:shadow-lg transition group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-cta/30 flex items-center justify-center">
